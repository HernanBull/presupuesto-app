...
export default function PublicStore() {
  ...
  const trackEvent = async (eventType, wid) => {
    try {
      ...
      if (!activeWId) return;
      await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/track`, {
        method: 'POST',
  ...
  const handleRegister = async (e) => {
    ...
    setAuthLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/customers/register`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleCheckoutSubmit = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/orders`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/upload`, {
        method: 'POST',
        body: formData
      });
      const data = await res.json();
  ...
  const handleLogin = async (e) => {
    ...
    setAuthLoading(true);
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/customers/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const checkGlobalStock = async (productId, neededQty) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/products/${productId}`);
  ...
  const sendStockAlertWithAntiSpam = (product, requestedQty, available) => {
    ...
    if (!lastAlertTime || (now - Number(lastAlertTime)) > 900000) { // 15 minutos de bloqueo
      ...
      fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/notifications`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          workspace_id: slug,
  ...
  const fetchProductReviews = async (productId, currentWorkspace) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/reviews?workspaceId=${currentWorkspace}`);
      if (res.ok) {
        const data = await res.json();
  ...
  const handleReviewSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/reviews`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleApplyDiscount = async () => {
    ...
    setDiscountError('');
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/promotions/validate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    const fetchStoreData = async () => {
      ...
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/workspaces/store/${slug || 'tienda-ejemplo'}`, { cache: 'no-store' });
        if (!res.ok) {
          ...
          if (savedStr) {
             ...
             const pRes = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/products?workspaceId=default_workspace`);
             if (pRes.ok) {
        ...
        const pRes = await fetch(`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}/api/ecommerce/products?workspaceId=${storeData.id}`);
        if (pRes.ok) {
          const pData = await pRes.json();
          const published = pData
             .filter(p => p.publish_status === 'Publicado')
          ...
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => setCurrentPage('home')}>
            {logoUrl ? (
              <div className="h-10 w-auto bg-white/5 rounded-xl border border-white/10 p-1 backdrop-blur-md">
                 <img src={`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}${logoUrl}`} alt="Logo" className="h-full w-auto object-contain rounded-lg" />
              </div>
            ) : (
                  ...
                  <div key={section.id} className="relative overflow-hidden min-h-[500px] md:min-h-[700px] flex items-center justify-center text-center px-6">
                    {heroUrl ? (
                      <div className="absolute inset-0 z-0">
                        <img src={`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}${heroUrl}`} alt="Cover" className="w-full h-full object-cover opacity-40 scale-105" />
                      ...
                      {recentProducts.length > 0 ? recentProducts.map((p, i) => (
                        ...
                        >
                           ...
                           <div className="w-full aspect-square bg-zinc-950 relative overflow-hidden p-6">
                             {p.image_url ? (
                               <img id={`product-img-${p.id}`} src={`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}${p.image_url}`} alt={p.name} className="w-full h-full object-contain transition-transform duration-700 group-hover:scale-110" />
                             ) : (
                ...
                {offerProducts.length > 0 ? offerProducts.map((p, i) => (
                  ...
                  >
                    ...
                    <div className="w-full aspect-[4/5] bg-zinc-950 relative overflow-hidden p-8 flex items-center justify-center">
                      {p.image_url ? (
                        <motion.img id={`product-img-${p.id}`} whileHover={{ scale: 1.15, rotate: 2 }} transition={{ duration: 0.6 }} src={`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}${p.image_url}`} alt={p.name} className="w-full h-full object-contain relative z-10 drop-shadow-2xl" />
                      ) : (
      ...
      <footer className="bg-zinc-950 border-t border-white/5 mt-auto relative z-10 py-12 px-6">
        <div className="max-w-[1400px] mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
           <div className="flex items-center gap-4">
             {logoUrl && <img src={`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}${logoUrl}`} alt="Logo" className="h-8 object-contain grayscale opacity-50" />}
            ...
            >
              <div className="md:w-1/2 bg-black relative flex items-center justify-center p-12 overflow-hidden">
                 ...
                 {selectedProduct.image_url ? (
                   <img id={`modal-img-${selectedProduct.id}`} src={`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}${selectedProduct.image_url}`} alt={selectedProduct.name} className="w-full h-full object-contain relative z-10 max-h-[60vh]" />
                 ) : (
                      ...
                      return (
                        ...
                        >
                          <div className="w-24 h-24 bg-black rounded-2xl overflow-hidden flex-shrink-0 flex items-center justify-center border border-white/5 relative shadow-inner">
                            {p.image_url ? (
                              <img src={`${import.meta.env.VITE_API_URL || 'https://axomarket.pagina.dev'}${p.image_url}`} alt={p.name} className="w-full h-full object-cover opacity-90 group-hover:opacity-100 transition-opacity group-hover:scale-110 duration-500" />
                            ) : (
