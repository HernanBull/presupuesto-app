...
export default function NotificationsManager() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      setLoading(true);
      const res = await fetch(`http://localhost:3001/api/ecommerce/notifications/${workspaceId}`);
      if (res.ok) {
  ...
  const markAsRead = async (id) => {
    try {
      const res = await fetch(`http://localhost:3001/api/ecommerce/notifications/${id}/read`, { method: 'PUT' });
      if (res.ok) {
  ...
  const handleAutoReplenish = async (n) => {
    try {
      ...
      const res = await fetch(`http://localhost:3001/api/ecommerce/products/${n.product_id}`);
      if (!res.ok) {
      ...
      if (product.stock > 0) {
        ...
        const patchRes = await fetch(`http://localhost:3001/api/ecommerce/products/${n.product_id}`, {
          method: 'PATCH',
